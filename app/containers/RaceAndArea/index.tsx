...
function RaceAndArea(props) {
  const { classes } = props;
  const xData = _.uniq(data.map(it => it.dieCountry));
  const legendData = _.uniq(data.map(it => it.Race));
  let newData = legendData.map(it => data.filter(ite => ite.Race ==it));
...
export default withStyles(styles as any)(RaceAndArea);

...
function RaceAndGender(props) {
  const { classes } = props;

  const xData = _.uniq(data.map(it => it.Race));
  ...
  let finalData = legendData.map( (item,index) =>{
    return xData.map(it =>{
      let dd = newData[index].filter(ite => ite.Race == it)
...
export default withStyles(styles as any)(RaceAndGender);
